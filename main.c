...
#define STATE_SEND_MODE 4
#define STATE_RECV_MODE 5
...
void DemoRun()
{
	...
	while (1)
	{
		...
		else if (cur_state == STATE_CONNECTED_INIT)
		{
			...
			// Set to next state
			cur_state = STATE_SEND_MODE;
		}
		else if (cur_state == STATE_SEND_MODE)
		{
			...
			else
			{
				...
				if (status == KYPD_SINGLE_KEY && (status != last_status || key != last_key))
				{
					...
					else if (key == 'D')
					{
						...
						cur_state = STATE_RECV_MODE;

						// Print "Receive Mode" message, wait for 0.5 seconds, and then clear screen.
						xil_printf("Recv Mode...\r\n");
						usleep(500000);
					}
					else if (key == 'F')
					{
						// Add space to screen w/ updating cursor.
						xil_printf(" ");

						// Add space to the MsgToSend buffer.
						strncat(MsgToSendBuf, " ", 1);
					}
					else
					{
		...
		else if (cur_state == STATE_RECV_MODE)
		{
			if (PmodBLE_IsConnected() == 0)
			{
				// Set cur_state to STATE_NOT_CONNECTED_INIT
